...
def get_schedule_data_for_week(grade, start_date):
    ...
    # 指定された週と学年のデータをフィルタリング
    schedule_data = {}
    for date in dates:
        schedule_data[date] = {}
    
    for entry in data:
        if entry['date'] in dates and entry['grade'] == grade:
            date = entry['date']
            period = entry['period']
...
def get_available_grades():
    ...
    # 利用可能な学年を取得
    grades = set(entry['grade'] for entry in data)
...
def main():
    ...
    # コマンドライン引数からパラメータを取得
    if len(sys.argv) < 2:
        print("使用法: python mh_week.py 学年 [YYYY-MM-DD]")
        print("例: python mh_week.py \"1年生\" 2025-04-07")
        return
